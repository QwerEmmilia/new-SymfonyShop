...
document.addEventListener('click', function(event) {
    ...
    var genderLabel = document.getElementById('gender-toggle');
    var genderOptions = document.getElementById('gender-options');

    if (target !== genderLabel && !genderOptions.contains(target)) {
...
function applyGender() {
    ...
    let genderOption = document.querySelector('input[name="gender-option"]:checked').value;
    ...
    if (genderOption && genderOption !== 'all') {
        params.push(`gender=${genderOption}`);
    }
    if (keyword) {
...
function applySorting() {
    ...
    let genderOption = getQueryParam('gender');
    let keyword = getQueryParam('keyword');
    let typeOption = getTypeFromPath();
    let url = '/search';
    if (typeOption) {
